...
def parallel_scan(x, log_decay, prev_state=None):
    B, H, T, K = x.shape
    ...
    if prev_state is not None:
        decay_from_start = torch.exp(decay_cumsum).unsqueeze(-1)
        y = y + (prev_state.unsqueeze(2) * decay_from_start)
    
    last_state = y[:, :, -1, :]
    return y, last_state

def recurrent_step(x, log_decay, prev_state):
    decay = torch.exp(log_decay).view(x.shape[0], x.shape[1], 1)
    
    current_state = decay * prev_state + x.squeeze(2)
    
    y = current_state.unsqueeze(2)
...
class XylemMixer(nn.Module):
    def __init__(self, dim, n_heads=8):
        ...
        self.local_conv = nn.Conv1d(dim, dim, kernel_size=4, groups=dim, padding=0)
    ...
    def forward(self, x, state=None):
        ...
        k_in = k.permute(0, 2, 1)
        conv_state_prev = state['conv'] if (state is not None and 'conv' in state) else None
        
        if conv_state_prev is not None:
            k_padded = torch.cat([conv_state_prev, k_in], dim=-1)
        ...
        if rnn_prev is None:
            rnn_prev = torch.zeros(B, H, K, device=x.device, dtype=v_in.dtype)

        if T > 1:
            y_out, next_rnn_state = parallel_scan(v_in, log_decay_in, rnn_prev)
        else:
            y_out, next_rnn_state = recurrent_step(v_in, log_decay_in, rnn_prev)
...
class SwiGLU(nn.Module):
    def __init__(self, dim, hidden_dim=None):
        super().__init__()
        if hidden_dim is None: hidden_dim = int(dim * 2.5)
...
class XuyenChi(nn.Module):
    ...
    def generate(self, prompt, max_new_tokens, temperature=1.0):
        ...
        logits, _, states = self(current_idx, states=None)
        
        next_token_logits = logits[:, -1, :] / temperature
        idx_next = torch.multinomial(F.softmax(next_token_logits, dim=-1), 1)
        current_idx = torch.cat((current_idx, idx_next), dim=1)
        
        for _ in range(max_new_tokens):
            logits, _, states = self(idx_next, states=states)
